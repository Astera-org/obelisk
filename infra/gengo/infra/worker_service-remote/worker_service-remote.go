...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  void sendResults(i32 job_id)")
  fmt.Fprintln(os.Stderr)
  os.Exit(0)
}
...
func main() {
  ...
  client := infra.NewWorkerServiceClient(thrift.NewTStandardClient(iprot, oprot))
  ...
  case "sendResults":
    if flag.NArg() - 1 != 1 {
      fmt.Fprintln(os.Stderr, "SendResults requires 1 args")
      flag.Usage()
    }
    ...
    argvalue0 := int32(tmp0)
    value0 := argvalue0
    fmt.Print(client.SendResults(context.Background(), value0))
    fmt.Print("\n")
    break
  case "":
    Usage()
