...
type AutoUI struct {
	...
	AddNetworkLoggingCallback func(userInterface *AutoUI) `desc:"If set, this adds logging based on the network structure. It's a callback because the function might need to live in the axon codebase, which can't be imported here for import loop reasons. Use axon.AddCommonLogItemsForOutputLayers if you're using an axon network."`
...
func (ui *AutoUI) CreateAndRunGuiWithAdditionalConfig(config func()) {
	...
	gimain.Main(func() { // this starts gui -- requires valid OpenGL display connection (e.g., X11)
		...
		if ui.Looper == nil {
			println("Looper is required by AutoUI")
			return
		}
...
func (ui *AutoUI) AddServerButton(serverRunFunc func()) {
...
func (ui *AutoUI) log(mode etime.Modes, time etime.Times, loop looper.Loop) {
...
func (ui *AutoUI) addDefaultLoggingCallbacksLooper() {
	manager := ui.Looper
...
//		}}})
func (ui *AutoUI) AddDefaultLogging() {
	if ui.Logs == nil {
		ui.Logs = &elog.Logs{}
	}
